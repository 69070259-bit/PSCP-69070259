...
```text
โจทย์ให้นำค่า RGB 2 อันมาผสมกัน และ พิมพ์สรที่ผสมได้ออกมา
```
...
```text
step1: สร้างตัวแปรรับค่า string "color1"
step2: สร้างตัวแปรรับค่า string "color2"
step3: R = float(color1[0]) * 0.5 + float(color2[0]) * 0.5
step4: G = float(color1[1]) * 0.5 + float(color2[1]) * 0.5
step5: B = float(color1[2]) * 0.5 + float(color2[2]) * 0.5
step6: พิมพ์ int(R), int(G), int(B)
```
